...
**La suite**  
Le package `sf` ([@R-sf], [@Pebesma18]) a été publié fin 2016 par Edzer Pebesma (auteur de `sp`). Son objectif est de combiner dans les fonctionnalités de `sp`, `rgeos` et `rgdal` dans un package unique plus ergonomique. 
...
On peut joindre d'autres données à un objet sf en utilisant la fonction `merge()`. 
...
Les objets `sf` **sont** des `data.frame`, on peut donc selectionner leur lignes et leur colonnes de la même manière que les `data.frame`. 
...
### Selection spatiale
Selection des communes interectants Fort-de-France
...
Le package `osmdata` [@R-osmdata] permet d'extraire des données depuis OSM. 
