...
library(sf)
# Import des données
mtq <- st_read(system.file("shape/martinique.shp", package="cartography"), quiet = 
                 TRUE)
# Communes
...
```{r shadow, echo=TRUE, eval=TRUE, prompt=FALSE, message=FALSE, fig.align="center",fig.width=4, fig.height=4.8, nm=TRUE}
...
`mapinsetr`[@R-mapinsetr] n'est pas sur le CRAN pour l'instant, son code source est sur GitHub et on peut l'installer via le package `remotes`. 
...
```{r, echo=TRUE, eval=TRUE, prompt=FALSE, message=FALSE, fig.align="center",fig.width=4, fig.height=4.8, nm=TRUE, cache=TRUE, result='hide'}
library(mapinsetr)
library(cartography)
library(sf)
# Import des données
...
zbox_FDF <- move_and_resize(x = box_FDF, mask = box_FDF, xy = c(689000, 1603000), 
                            k = 7)
zmtq_FDF <- move_and_resize(x = mtq, mask = box_FDF, xy = c(689000, 1603000), 
                            k = 7)
zresto_FDF <- move_and_resize(x = resto, mask = box_FDF, xy = c(689000, 1603000), 
                              k = 7)
