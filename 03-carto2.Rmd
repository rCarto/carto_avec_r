...
mapview(
  resto, map.types = "OpenStreetMap", col.regions = "#940000", 
  label = resto$name,alpha.regions=90,color = "white", legend = TRUE, 
  layer.name = "Restaurants", 
  homebutton = FALSE, lwd = 0.5, 
  popup = NA
) +
  mapview(
    mtq, col.regions = "lightblue", color = "white", legend = TRUE, 
    label = mtq$LIBGEO, alpha.regions= 0.5, map.types = "OpenStreetMap",
    lwd = 0.5, layer.name = "Communes",  homebutton = FALSE, 
    popup = popupTable(mtq, zcol = 1:4, row.numbers = FALSE, 
                       feature.id = FALSE)
  )
...
discLayer(
  ...
  var = "emp_share", 
  col="darkred", 
  nclass=3, method="quantile", 
  threshold = 0.5, 
  sizemin = 0.5, sizemax = 8, 
  type = "abs", 
  legend.values.rnd = 1,
  legend.title.txt = "Discontinuités\n(différences absolues)",
  legend.pos = "bottomleft", add=TRUE
)
layoutLayer("Discontinuités - Part des employés",tabtitle=TRUE, 
            author= "INSEE 2016", sources="", frame=FALSE, scale = 5)
north(pos = "topleft")
```
...
layoutLayer(title = "Répartition des restaurants",tabtitle=TRUE,
            author= "INSEE, 2016 - OSM, 2018", sources="", 
            frame=FALSE, scale = 5)
north(pos = "topleft")
...
mtq <- st_read("data/martinique.shp", quiet = TRUE)# use WGS84 proj
...
# contour extraction
mtq_cont <- rasterToContourPoly(r = mtq_ras, breaks = bv, 
                                mask = as(mtq, "Spatial"))
# custom palette
...
legendChoro(pos = c(689000,1598000 ), breaks = bv, col = pal, nodata = F,
            title.txt = "Elevation\n(metres)", cex = 1)
layoutLayer(title = "Martinique Relief", north = T,
            sources = 'T. Giraud, 2018', author = "SRTM, 2018", 
            col = "lightblue", 
